...
defmodule ActiveJob.Arguments do
  ...
  # GlobalID.
  def serialize(arguments) do
    arguments |> Enum.map(fn argument -> serialize_argument(argument) end)
  end
  ...
  # GlobalID.
  def deserialize(arguments) do
    arguments |> Enum.map(fn argument -> deserialize_argument(argument) end)
    # rescue
  ...
  #   GLOBALID_KEY.to_sym(),
  #   SYMBOL_KEYS_KEY,
  #   SYMBOL_KEYS_KEY.to_sym(),
  #   RUBY2_KEYWORDS_KEY,
  #   RUBY2_KEYWORDS_KEY.to_sym(),
  #   OBJECT_SERIALIZER_KEY,
  #   OBJECT_SERIALIZER_KEY.to_sym(),
  ...
  def serialize_argument(argument) do
    # case argument
  ...
  def deserialize_argument(argument) do
    # case argument
    # when String
    #   argument
    # when *PERMITTED_TYPES
  ...
  def serialized_global_id?(hash) do
    # hash.size == 1 && hash.include?(GLOBALID_KEY)
  end

  def deserialize_global_id(hash) do
    # GlobalID::Locator.locate hash[GLOBALID_KEY]
  end

  def custom_serialized?(hash) do
    # hash.key?(OBJECT_SERIALIZER_KEY)
  end

  def serialize_hash(argument) do
    # argument.each_with_object({}) do |(key, value), hash|
    #  hash[serialize_hash_key(key)] = serialize_argument(value)
    # end
  end

  def deserialize_hash(serialized_hash) do
  ...
  def serialize_indifferent_hash(indifferent_hash) do
    # result = serialize_hash(indifferent_hash)
    # result[WITH_INDIFFERENT_ACCESS_KEY] = serialize_argument(true)
    # result
  end

  def transform_symbol_keys(hash, symbol_keys) do
  ...
  def convert_to_global_id_hash(argument) do
